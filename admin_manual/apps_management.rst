...
cyfrSpaces apps can enhance, customize or even restrict the features and experience
you and your users has with the cyfrSpaces server. Next to default enabled functions
like Files, Activity and Gallery there are other apps like Calendar, Contacts,
Talk and more which are enhancing the features of your cyfrSpaces server.

After installing the cyfrSpaces server, you might want to consider about enabling,
...
During the cyfrSpaces server installation, some apps are enabled by default.
To see which apps are enabled go to your Apps page.

Those apps are supported and developed by cyfrSpaces GmbH directly and
...
.. note:: To get access to work-arounds, long-term-support, priority bug fixing
          and custom consulting for supported apps, contact our `sales team <https://cyfr.space/enterprise/>`_.

Apps with **Approved** tag are community-developed and were uploaded in the
`cyfrSpaces App Store <https://apps.nextcloud.com>`_.

.. note:: If you would like to develop your own cyfrSpaces app, you can find out
...
your cyfrSpaces settings (admin and/or user-settings) or in the ``config.php``.
...
If the app is not part of the cyfrSpaces installation, it will be downloaded from
...
the cyfrSpaces web root. The key **writable** indicates if a user can install apps
in that folder.
...
2. Set the **appstoreurl** to the URL of your cyfrSpaces apps store.

   This parameter is used to set the http path to the cyfrSpaces apps store. The appstore
