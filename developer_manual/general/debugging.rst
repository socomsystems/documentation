...
cyfrSpaces uses custom error PHP handling that prevents errors being printed to Web server log files or command line output. Instead, errors are generally stored in Nextcloud's own log file, located at: :file:`/data/nextcloud.log`
...
.. code-block:: php
  ...
  <?php throw new \Exception( "\$user = $user" ); // should be logged in cyfrSpaces ?>
...
To disable custom error handling in cyfrSpaces (and have PHP and your Web server handle errors instead), see Debug mode.
...
Once you are familiar with how your debugging client works, you can start debugging with XDebug. To test cyfrSpaces through the web interface or other HTTP requests, set the ``XDEBUG_SESSION_START`` cookie or POST parameter. Alternatively, there are browser extensions to make this easy:
...
By default all JavaScript files in cyfrSpaces are minified (compressed) into a single file without whitespace. To prevent this, see Debug mode.


Debugging HTML and templates
----------------------------

By default cyfrSpaces caches HTML generated by templates. This may prevent changes to app templates, for example, from being applied on page refresh. To disable caching, see Debug mode.
